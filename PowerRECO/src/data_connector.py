...
def parse_manager_output(file_obj) -> pd.DataFrame:
    ...
    Column names are lower-cased and spaces/hyphens replaced with underscores.
    A 'timestamp' column is parsed to datetime when available.
    """
...
def _json_to_df(raw) -> pd.DataFrame:
    ...
    if isinstance(raw, dict):
        ...
        # Flat dict of arrays (column-oriented)
        try:
            return pd.DataFrame(raw)
        except Exception:
            return pd.json_normalize(raw)
    raise ValueError("JSON structure not recognised — expected a list or a dict with a results/data key.")


def _read_csv_flexible(file_obj) -> pd.DataFrame:
    """Try multiple delimiters so the user doesn't have to care."""
...
def extract_peak_demand_stats(df: pd.DataFrame) -> dict:
    ...
    Detects the best available kW column automatically.
    """
    kw_col = None
