...
_MY_PUBLIC_HOLIDAYS: frozenset = frozenset({
    ...
    datetime.date(2027, 6, 7),   # Yang DiPertuan Agong Birthday
...
def calculate_bill(
    ...
    if tariff == "A":
        ...
        result["energy_charge"] = ec
        result["md_charge"]     = round(max_demand_kva * TARIFF_C1_MD, 2)
        result["blocks"] = [{"Block": "All kWh (flat rate)",
        ...
        result["md_charge"]     = round(max_demand_kva * TARIFF_C2_MD, 2)
        result["blocks"] = [
        ...
        result["energy_charge"] = ec
        result["md_charge"]     = round(max_demand_kva * TARIFF_D_MD, 2)
        result["blocks"] = [{"Block": "All kWh (flat rate)",
        ...
        result["energy_charge"] = ep + eo
        result["md_charge"]     = round(max_demand_kva * TARIFF_E1_MD, 2)
        result["blocks"] = [
            {"Block": "Peak kWh  (08:00–22:00, Mon–Sat)",
...
def compute_monthly_stats(df: pd.DataFrame, interval_minutes: int = 30) -> pd.DataFrame:
    ...
    # TNB measures MD as the highest 30-min kVA reading: √(kW² + kVAR²)
    df["kva_import"]    = np.sqrt(df["kw_import"]**2 + df["kvar_import"]**2)
    ...
    stats = df.groupby("month", group_keys=False).apply(
        lambda g: pd.Series({
            ...
            "max_demand_kva": round(float(g["kva_import"].max()), 3),
